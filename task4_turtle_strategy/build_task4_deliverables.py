...
def infer_instrument_key(path: Path) -> str:
    stem = path.stem.split("_daily_")[0]
    parts = stem.split("_")
    if len(parts) >= 3 and parts[-2].isdigit() and parts[-1] in {"SZ", "SH", "HK"}:
        return "_".join(parts[:-2])
    return stem


def infer_market_currency(ts_code: str) -> tuple[str, str]:
...
def discover_data_files() -> list[Path]:
    processed = sorted(DATA_DIR.glob("*_daily_*.csv"))
    legacy = sorted((PROJECT_ROOT / "data").glob("*_daily_*.csv"))
    seen: set[Path] = set()
    files: list[Path] = []
    for path in [*processed, *legacy]:
        if "three_companies" in path.name or path in seen:
            continue
        seen.add(path)
        files.append(path)
    return files
...
def read_stock_data(path: Path) -> pd.DataFrame:
    ...
    market, currency = infer_market_currency(ts_code)
    if "instrument_key" not in data.columns:
        data["instrument_key"] = infer_instrument_key(path)
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .plotly-chart { width:100%; height:330px; }
    .chart-block.large .plotly-chart { height:460px; }
...
<body>
    ...
    <main class="workspace">
      ...
      <section class="chart-panel">
        ...
        <div class="chart-grid">
          <section class="chart-block large"><div class="chart-block-head"><h3>复权价格、高低通道与交易信号</h3><span id="priceLabel" class="muted"></span></div><div id="priceChart" class="plotly-chart" role="img" aria-label="价格通道图"></div></section>
          <section class="chart-block"><div class="chart-block-head"><h3>策略净值、基准与回撤</h3><span id="equityLabel" class="muted"></span></div><div id="equityChart" class="plotly-chart" role="img" aria-label="净值回撤图"></div></section>
          <section class="chart-block"><div class="chart-block-head"><h3>参数敏感性</h3><span class="muted">当前标的 · 4 组通道参数</span></div><div id="sensChart" class="plotly-chart" role="img" aria-label="参数敏感性图"></div></section>
        </div>
  ...
  <script>
    ...
    function plotlyReady(){ return typeof Plotly !== "undefined"; }
    function emptyChart(id,message){ document.querySelector(id).innerHTML=`<p class="small-copy" style="padding:18px;">${message}</p>`; }
    function drawPrice(dataset,rows,cfg){ if(!plotlyReady()){ emptyChart("#priceChart","Plotly 图表库暂未加载。"); return; } const visible=rows.slice(-190); const buyRows=visible.filter(r=>r.tradeAction==="BUY"); const sellRows=visible.filter(r=>r.tradeAction==="SELL"); const traces=[{x:visible.map(r=>r.date),y:visible.map(r=>r.close),name:"复权收盘价",type:"scatter",mode:"lines",line:{color:"#213547",width:2.4},hovertemplate:"%{x}<br>收盘价 %{y:.2f}<extra></extra>"}]; if(document.querySelector("#showEntry").checked) traces.push({x:visible.map(r=>r.date),y:visible.map(r=>r.entryHigh),name:`${cfg.entryWindow}日入场高点`,type:"scatter",mode:"lines",line:{color:"#d9822b",width:1.8},hovertemplate:"%{x}<br>入场通道 %{y:.2f}<extra></extra>"}); if(document.querySelector("#showExit").checked) traces.push({x:visible.map(r=>r.date),y:visible.map(r=>r.exitLow),name:`${cfg.exitWindow}日退出低点`,type:"scatter",mode:"lines",line:{color:"#0f766e",width:1.8},hovertemplate:"%{x}<br>退出通道 %{y:.2f}<extra></extra>"}); traces.push({x:visible.map(r=>r.date),y:visible.map(r=>r.stopPrice),name:`${cfg.stopN}N 止损线`,type:"scatter",mode:"lines",line:{color:"#b94d4d",width:1.4,dash:"dash"},hovertemplate:"%{x}<br>止损线 %{y:.2f}<extra></extra>"}); if(document.querySelector("#showSignals").checked){ traces.push({x:buyRows.map(r=>r.date),y:buyRows.map(r=>r.close),name:"买入",type:"scatter",mode:"markers",marker:{symbol:"triangle-up",size:12,color:"#2e8f57",line:{color:"#fff",width:1}},text:buyRows.map(r=>r.tradeReason),hovertemplate:"买入<br>%{x}<br>价格 %{y:.2f}<br>%{text}<extra></extra>"}); traces.push({x:sellRows.map(r=>r.date),y:sellRows.map(r=>r.close),name:"卖出",type:"scatter",mode:"markers",marker:{symbol:"triangle-down",size:12,color:"#b94d4d",line:{color:"#fff",width:1}},text:sellRows.map(r=>r.tradeReason),hovertemplate:"卖出<br>%{x}<br>价格 %{y:.2f}<br>%{text}<extra></extra>"}); } Plotly.react("priceChart",traces,{height:450,template:"plotly_white",hovermode:"x unified",margin:{l:58,r:24,t:28,b:42},legend:{orientation:"h",yanchor:"bottom",y:1.02,xanchor:"left",x:0},xaxis:{rangeslider:{visible:true,thickness:.08}},yaxis:{title:"价格"}},plotConfig); }
    function drawEquity(rows){ if(!plotlyReady()){ emptyChart("#equityChart","Plotly 图表库暂未加载。"); return; } const traces=[{x:rows.map(r=>r.date),y:rows.map(r=>r.portfolio),name:"策略净值",type:"scatter",mode:"lines",line:{color:"#1f6fb2",width:2.3},xaxis:"x",yaxis:"y",hovertemplate:"%{x}<br>策略净值 %{y:.2f}<extra></extra>"}]; if(document.querySelector("#showBenchmark").checked) traces.push({x:rows.map(r=>r.date),y:rows.map(r=>r.benchmark),name:"买入持有",type:"scatter",mode:"lines",line:{color:"#64748b",width:1.8,dash:"dash"},xaxis:"x",yaxis:"y",hovertemplate:"%{x}<br>买入持有 %{y:.2f}<extra></extra>"}); if(document.querySelector("#showDrawdown").checked) traces.push({x:rows.map(r=>r.date),y:rows.map(r=>r.drawdown),name:"回撤",type:"scatter",mode:"lines",fill:"tozeroy",fillcolor:"rgba(185,77,77,.18)",line:{color:"#b94d4d",width:1.5},xaxis:"x2",yaxis:"y2",hovertemplate:"%{x}<br>回撤 %{y:.2%}<extra></extra>"}); Plotly.react("equityChart",traces,{height:320,template:"plotly_white",hovermode:"x unified",margin:{l:58,r:24,t:18,b:42},legend:{orientation:"h",yanchor:"bottom",y:1.02,xanchor:"left",x:0},grid:{rows:2,columns:1,pattern:"independent",roworder:"top to bottom"},xaxis:{domain:[0,1],anchor:"y"},yaxis:{domain:[.34,1],title:"净值"},xaxis2:{domain:[0,1],anchor:"y2"},yaxis2:{domain:[0,.22],title:"回撤",tickformat:".0%"}},plotConfig); }
    function drawSensitivity(dataset,cfg){ if(!plotlyReady()){ emptyChart("#sensChart","Plotly 图表库暂未加载。"); return; } const combos=[[20,10],[30,15],[40,20],[55,20]]; const base=filteredRows(dataset); const rows=combos.map(([entryWindow,exitWindow])=>{ const localCfg={...cfg,entryWindow,exitWindow}; return {parameter:`${entryWindow}/${exitWindow}`,...metrics(backtest(base,localCfg),localCfg)}; }); const colors=rows.map(r=>r.cumulative>=0?"#9fd8b6":"#e9a8a8"); const traces=[{x:rows.map(r=>r.parameter),y:rows.map(r=>r.cumulative),name:"累计收益",type:"bar",marker:{color:colors},customdata:rows.map(r=>[r.maxDrawdown,r.tradeCount,r.benchmarkCumulative]),hovertemplate:"参数 %{x}<br>累计收益 %{y:.2%}<br>最大回撤 %{customdata[0]:.2%}<br>交易次数 %{customdata[1]}<br>买入持有 %{customdata[2]:.2%}<extra></extra>"}]; Plotly.react("sensChart",traces,{height:310,template:"plotly_white",margin:{l:58,r:24,t:20,b:42},yaxis:{title:"累计收益",tickformat:".0%"},xaxis:{title:"入场/退出通道"}},plotConfig); }
